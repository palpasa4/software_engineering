...
$limit=4;
$offset=($page-1)*$limit; //offset vaneko each naya page ma kun page no value bata start garne vanne value
$sql="SELECT * FROM blog LEFT JOIN categories ON blog.category=categories.cat_id 
LEFT JOIN user ON blog.author_id=user.user_id WHERE blog_title LIKE '%$keyword%' OR blog_body LIKE '%$keyword%' ORDER BY blog.publish_date DESC limit $offset,$limit";
...
<div class="container mt-2">
	<div class="row">
		<h3 class="mb-0">Search results for: <span class="text-primary"><?=$keyword?></span></h3>
		<div class="col-lg-8">
			<hr>
            <?php 
                if($row)
				...
				<div class="card-body d-flex blog_flex">
					...
					<div class="flex-grow-1 flex-part2">
						...
						<p>
						  ...
						  <?= ucfirst(strip_tags(substr($result['blog_body'],0,200)))."<span class='text-primary fw-bold'>...more</span>" ?>
						  </a> <span><br>
                          <a href="single_post.php?id=<?= $result['blog_id'] ?>" class="btn btn-sm btn-outline-primary mt-5">Continue Reading
                          </a></span>
                        </p>
						<ul>
							...
							<li>
								<a href="category.php?id=<?= $result['cat_id'] ?>"> <span><i class="fa fa-tag" aria-hidden="true"></i></span> 
								<?= $result['cat_name'] ?>
							</a>
						    </li>
						</ul>
			...
			<?php
			$pagination="SELECT * FROM blog WHERE blog_title LIKE
			'%$keyword%' OR blog_body LIKE '%$keyword%'";
